...
@interface CardDetailViewController () <UINavigationControllerDelegate, SRActionSheetDelegate, UIGestureRecognizerDelegate, UITableViewDataSource, UITableViewDelegate, ESPictureBrowserDelegate, ReplyCommentContentViewDelegate>
...
@property (strong, nonatomic) NSString *otherCenterID;
...
//    self.navigationController.interactivePopGestureRecognizer.enabled = NO;
    ...
    _commentArray = [[NSMutableArray alloc] initWithCapacity:20];
    _replyCommentArray = [[NSMutableArray alloc] initWithCapacity:20];
    
    _deliverArray = [[NSMutableArray alloc] initWithCapacity:20];
    _collectArray = [[NSMutableArray alloc] initWithCapacity:20];
    
    _viewHeightConstraint.constant = SCREEN_HEIGHT + 238;
...
//    _tableScrollView.contentSize = CGSizeMake(_viewWidthConstraint.constant, _viewHeightConstraint.constant);
    ...
    _tableScrollView.scrollEnabled = NO;
    ...
    //添加这两行代码来自适应label的高度
    self.commentTableView.estimatedRowHeight = 44.0f;
    ...
    self.contentViewFrame = CGRectMake(0, 0, 0, 0);
...
- (void)requestCommentWithOffset:(NSInteger)offset {
    ...
    [commentListApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
        if ([weakSelf isSuccessWithRequest:request.responseJSONObject]) {
            ...
            NSString *totalCommentCount = request.responseJSONObject[@"data"][@"commentCount"];
            ...
            if (totalCommentCount.integerValue == weakSelf.commentArray.count) {
...
- (void)requestFavoriteUsersWithOffset:(NSInteger)offset {
    ...
    [favoriteUsersApi startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
        if ([weakSelf isSuccessWithRequest:request.responseJSONObject]) {
            ...
            UIView *noPublishView = [weakSelf.collectTableView viewWithTag:2016];
            if (noPublishView) {
                [noPublishView removeFromSuperview];
            }
            
            if (weakSelf.collectArray.count == 0 && favoriteCount.integerValue  == 0) {
...
- (void)commentRequestWithComment:(NSString *)comment {
    ...
    [commentApi startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
        if ([weakSelf isSuccessWithRequest:request.responseJSONObject]) {
            ...
            weakSelf.recommendModel.commentTimes = [NSString stringWithFormat:@"%td", weakSelf.recommendModel.commentTimes.integerValue + 1];
            [weakSelf.commentButton setTitle:[NSString stringWithFormat:@"留言 %@", weakSelf.recommendModel.commentTimes]
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _commentTableView) {
        ...
        commentCell.replyCommentContentView.replyDelegate = self;
        ...
        commentCell.commentContentButtonTapBlock = ^(CommentModel *commentModel) {
