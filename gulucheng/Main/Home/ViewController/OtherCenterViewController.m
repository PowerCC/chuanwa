...
- (void)viewDidLoad {
    ...
    [userInfoApi startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
        if ([weakSelf isSuccessWithRequest:request.responseJSONObject]) {
            ...
            UserModel *userModel = [UserModel JCParse:request.responseJSONObject[@"data"]];
            
            if (!weakSelf.navigationItem.title) {
                weakSelf.title = userModel.nickName;
            }
            
            weakSelf.nickNameLabel.text = userModel.nickName;
            weakSelf.locationLabel.text = [NSString stringWithFormat:@"常在地区：%@", GlobalData.userModel.cityCode];
            weakSelf.totalCardLabel.text = userModel.events;
            weakSelf.sendAroundLabel.text = userModel.sendAround;
            
            [weakSelf.headBlueImageView sd_setImageWithURL:[NSURL URLWithString:userModel.avatar]
                                  placeholderImage:nil];
            [weakSelf.headImageView sd_setImageWithURL:[NSURL URLWithString:userModel.avatar]
                              placeholderImage:[UIImage imageNamed:@"home-commentDefaultHead"]];
            
            weakSelf.genderImageView.image = [UIImage imageNamed:userModel.gender.integerValue == 1 ? @"myCenter-boy" : @"myCenter-girl"];
            
            weakSelf.totalCardCountLabel.text = userModel.events;
            weakSelf.photoCardCountLabel.text = [NSString stringWithFormat:@"%@张", userModel.picEvents];
            weakSelf.textCardCountLabel.text = [NSString stringWithFormat:@"%@张", userModel.textEvents];
            weakSelf.voteCardCountLabel.text = [NSString stringWithFormat:@"%@张", userModel.voteEvents];
            
            NSArray *eventArray = @[userModel.picEvents, userModel.textEvents, userModel.voteEvents];
            NSInteger maxEvent = [[eventArray valueForKeyPath:@"@max.intValue"] integerValue];
            
            if (maxEvent != 0) {
                weakSelf.photoConstraint.constant = userModel.picEvents.floatValue/maxEvent * 145;
                weakSelf.textConstraint.constant = userModel.textEvents.floatValue/maxEvent * 145;
                weakSelf.voteConstraint.constant = userModel.voteEvents.floatValue/maxEvent * 145;
            }
        }
