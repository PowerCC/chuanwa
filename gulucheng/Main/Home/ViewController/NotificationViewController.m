...
//        [weakSelf requestMorePublishWithType:2 offset:limit * ++ _historyPage];
        [weakSelf requestMyEventNoticeListWithType:2 ownFlag:1 offset:limit * ++ weakSelf.historyPage];
    }];
...
//            NSArray *notificationArray = [RecommendModel JCParse:request.responseJSONObject[@"data"][@"list"]];
            ...
            if (currentFlag == 1) {
                ...
                if (weakSelf.lastMessagePage == 0) {
                    [weakSelf.lastMessageArray removeAllObjects];
                    [weakSelf.publishedCollectionView setContentOffset:CGPointMake(0, 0)];
                }
                
//                for (UserEventVoModel *model in notificationArray) {
//                    if (model.lastNoticeTimes.integerValue > 0) {
...
//                    [AppDelegateInstance.homeViewController.notificationButton setImage:[UIImage imageNamed:@"home-notice"] forState:UIControlStateNormal];
            ...
            if (currentFlag == 2) {
                ...
                if (weakSelf.historyPage == 0) {
                    [weakSelf.historyArray removeAllObjects];
                    [weakSelf.markCollectionView setContentOffset:CGPointMake(0, 0)];
                }
...
//            NSArray *notificationArray = [RecommendModel JCParse:request.responseJSONObject[@"data"][@"list"]];
            ...
            if (currentFlag == 1) {
                ...
                if (weakSelf.otherPeopleNewsPage == 0) {
                    [weakSelf.otherPeopleNewsArray removeAllObjects];
                    [weakSelf.otherPeopleNewsCollectionView setContentOffset:CGPointMake(0, 0)];
                }
...
//                    [AppDelegateInstance.homeViewController.notificationButton setImage:[UIImage imageNamed:@"home-notice"] forState:UIControlStateNormal];
            ...
            if (currentFlag == 2) {
                ...
                if (weakSelf.historyNewsPage == 0) {
                    [weakSelf.historyNewsArray removeAllObjects];
                    [weakSelf.historyNewsCollectionView setContentOffset:CGPointMake(0, 0)];
                }
...
#pragma mark - requestCommentWithEid 留言确认请求
- (void)requestCommentWithEid:(NSString *)eid {
    CommentListApi *commentListApi = [[CommentListApi alloc] initWithEid:eid
                                                                  offset:0
                                                           fromNewNotice:YES];
...
//                [AppDelegateInstance.homeViewController.notificationButton setImage:[UIImage imageNamed:@"home-notice"] forState:UIControlStateNormal];
        ...
        [self requestCommentWithEid:userEventVoModel.eid];
