...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    ...
    if ([self _canRecord]) {
        ...
        tmpView.center = CGPointMake(self.view.center.x, self.view.center.y - 94);
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            if ([error.domain isEqualToString:@"录制时间过短"] || error.code == -100) {
                UIView *errorInfoView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 154, 154)];
                errorInfoView.center = CGPointMake(self.view.center.x, self.view.center.y - 94);
                errorInfoView.backgroundColor = kCOLOR(0, 0, 0, 0.6);
                errorInfoView.layer.cornerRadius = 5;
                errorInfoView.layer.masksToBounds = YES;
                
                UIImageView *shortTimeImageView = [[UIImageView alloc] initWithFrame:CGRectMake((errorInfoView.bounds.size.width - 45) / 2, 40, 45, 45)];
                shortTimeImageView.image = [UIImage imageNamed:@"im-record-time"];
                shortTimeImageView.contentMode = UIViewContentModeScaleAspectFit;
                [errorInfoView addSubview:shortTimeImageView];
                
                UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake((errorInfoView.bounds.size.width - 100) / 2, errorInfoView.bounds.size.height - 60, 100, 25)];
                label.text = @"说话时间太短";
                label.textColor = [UIColor whiteColor];
                label.font = [UIFont systemFontOfSize:14];
                label.textAlignment = NSTextAlignmentCenter;
                [errorInfoView addSubview:label];
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (self.messageTimeIntervalTag < 0 || interval > 60 || interval < -60) {
            ...
            timeStr = model.isSender ? [NSString stringWithFormat:@"##send_%@", timeStr] : timeStr;
...
{
    NSDictionary *ext = @{@"nickname": _conversationUserModel.nickName, @"photo": _conversationUserModel.avatar, @"gender": _conversationUserModel.gender};
    if (self.conversation.type == EMConversationTypeGroupChat) {
